...
double eucl(double *aux1, double *aux2,int n_dim){
    //returns s
    double ret=0.0;
    double aux=0.0;
...
double inner(double *a, double *b,int n_dim){
    double ret=0.0;
    for(int i=0;i < n_dim; i++){
        ret+=a[i]*b[i];
    }
...
double* median_center( double **orth_aux,long size,int n_dim){
...
double rad(double** data, double* center,long data_size,int n_dim){
    double ret=0;
    for(long i=0; i<data_size;i++){
        double aux=eucl(data[i],center,n_dim);
        if(aux>ret){
            ret=aux;
        }
...
void fit(struct tree *node, double** dataset, long size,int n_dim,long id){
    node->id=id;
    if(size<=1){
        ...
        for(long i=1;i<size;i++){
            temp_dist=eucl(dataset[0],dataset[i],n_dim);
        ...
        temp_dist=eucl(dataset[a],dataset[j],n_dim);
            if(temp_dist>aux_dist){
                aux_dist=temp_dist;
                b=j;
            }
        ...
        double inner_b_a=inner(b_a,b_a,n_dim);
        ...
        for(long i=0;i < size ;i++){
            ...
            aux=inner(sub_aux,b_a,n_dim)/inner_b_a;
    ...
    node->center=median_center(orth_aux,size,n_dim);

    node->rad=rad(dataset,node->center,size,n_dim);
    ...
    fit(node->L,ret1,aux1,n_dim,2*id+1);

    fit(node->R,ret2,aux2,n_dim,2*id+2);
...
void visit(struct tree *node,n_dim) {
...
void traverse(struct tree *node,int n_dim) {
  ...
  if(node->rad ==0.0){
      ...
      traverse(node->L,n_dim);

      visit(node,n_dim);

      traverse(node->R,n_dim);
